...
import { checkBridge, resetBridge, smartFetch } from '../../lib/bridge'
import {
  AI_GUIDES,
  AI_PROVIDERS,
  IMAGE_GUIDES,
...
async function fetchModels(config) {
  ...
  if (aiProvider === 'ollama') {
    const response = await smartFetch(`${aiUrl}/api/tags`)
    if (!response.ok) throw new Error(`Ollama ${response.status}`)
    return (await response.json()).models?.map((model) => ({ id: model.name, label: model.name })) ?? []
  }

  if (aiProvider === 'lmstudio') {
    const response = await smartFetch(`${aiUrl}/v1/models`, {
...
export default function ConfigStep() {
  ...
  const [bridgeStatus, setBridgeStatus] = useState(null) // null=checking, true=ok, false=absent
  ...
  // Check bridge availability whenever a local provider is selected from a hosted origin
  useEffect(() => {
    if (!showCorsWarning) {
    ...
    setBridgeStatus(null)
    checkBridge(1500).then(setBridgeStatus)
  }, [showCorsWarning, config.aiProvider])
                ...
                <AnimatePresence mode="wait">
                  {bridgeStatus === null && (
                    <motion.div
                      ...
                      animate={{ opacity: 1 }}
                      exit={{ opacity: 0 }}
                      className="flex items-center gap-2 rounded-xl bg-slate-50 border border-slate-200 px-3 py-2"
                    >
                      <span className="w-3.5 h-3.5 border border-slate-400 border-t-transparent rounded-full animate-spin flex-shrink-0" />
                  ...
                  {bridgeStatus === true && (
                    <motion.div
                      ...
                      exit={{ opacity: 0 }}
                      className="flex items-center gap-2 rounded-xl bg-green-50 border border-green-200 px-3 py-2.5"
                    >
                      <CheckCircle className="w-4 h-4 text-green-600 flex-shrink-0" />
                      <p className="text-xs text-green-700">{t('config.bridgeActive')}</p>
                    </motion.div>
                  )}
                  {bridgeStatus === false && (
                    <motion.div
